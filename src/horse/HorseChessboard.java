...
public class HorseChessboard {
    ...
    private static void traversalChessboard(int[][] chessboard, int row, int column, int step) {
        ...
        //获取当前位置可以走的下一个位置的集合
        ArrayList<Point> ps = nextStep(new Point(column, row));

        //优化：对ps进行排序,排序的规则就是对ps的所有的Point对象的下一步的位置的数目，进行非递减排序
        // 不进行优化，花费时间会很多
        sort(ps);
    ...
    /**
     * 功能： 根据当前位置(Point对象)，计算马儿还能走哪些位置(Point)，并放入到一个集合中(ArrayList)
     * 最多有8个位置
     * @param curPoint
     * @return
     */
    private static ArrayList<Point> nextStep(Point curPoint) {
        //创建一个ArrayList
